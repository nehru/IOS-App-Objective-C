...
@synthesize TwitterTable = _TwitterTable;
...
- (void)viewDidLoad
{
    ...
    self.TwitterTextView.layer.borderColor = [[UIColor grayColor]CGColor];
    
    self.TwitterTextView.delegate = self;
    
    
    self.TwitterTable.layer.borderWidth = 1.0;
    self.TwitterTable.layer.borderColor = [[UIColor grayColor]CGColor];
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
        if(screenSize.height > 480.0f){
            ...
            activity2.center = CGPointMake(self.view.center.x,self.view.center.y-225);
            ...
            activity2 = [[UIActivityIndicatorView alloc]initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
            activity2.frame = CGRectMake(0.0, 0.0, 40.0, 40.0);
            activity2.center = CGPointMake(self.view.center.x,self.view.center.y-180);
            activity2.color =[UIColor blueColor];
            [self.view addSubview:activity2];
   ...
   // NSLog(@"title = %@",[self.currentPhoto title]);
...
-(void)textViewDidChange:(UITextView *)textView{
    ...
    {
        textView.text = [tmp substringToIndex:[tmp length]-1];
    }
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if([segue.identifier isEqualToString:@"TwitterDetail"]){
        ...
        NSIndexPath *path = [self.tweetTableView indexPathForSelectedRow];
        Photo *tt = [photos objectAtIndex:[path row]];
...
- (IBAction)PostMessageAndPhoto:(id)sender {
 ...
 //   __block BOOL status = false;
    ...
    NSString *testMessage = self.TwitterTextView.text;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TwitterAppCell *cell = [self.tweetTableView
    ...
    // NSLog(@"log =%@",tweet[@"media.expanded_url"]);
    
    //ok
    //NSString *pt = [[[[tweet objectForKey:@"entities"]objectForKey:@"media"]objectAtIndex:0]objectForKey:@"media_url"];
    
    
    //NSString *pt1 = [[[[tweet objectForKey:@"entities"]objectForKey:@"user"]objectAtIndex:0]objectForKey:@"profile_image_url"];
    
    //NSLog(@"pt1 = %@",pt1);
    
    Photo *tt = [photos objectAtIndex:[indexPath row]];
